...
struct UsageStoreHighestUsageTests {
    ...
    @Test
    func `automatic metric keeps fully exhausted kimi eligible for highest usage`() {
        let settings = SettingsStore(
            configStore: testConfigStore(suiteName: "UsageStoreHighestUsageTests-kimi-fully-exhausted"),
            zaiTokenStore: NoopZaiTokenStore(),
            syntheticTokenStore: NoopSyntheticTokenStore())
        settings.refreshFrequency = .manual
        settings.statusChecksEnabled = false
    ...
    @Test
    func `automatic metric still excludes fully exhausted cursor while exhausted opt in kimi ranks`() {
        let settings = SettingsStore(
            configStore: testConfigStore(suiteName: "UsageStoreHighestUsageTests-kimi-exhausted-cursor-excluded"),
            zaiTokenStore: NoopZaiTokenStore(),
            syntheticTokenStore: NoopSyntheticTokenStore())
        settings.refreshFrequency = .manual
        settings.statusChecksEnabled = false
        ...
        let cursorOnly = store.providerWithHighestUsage(candidateProviders: [.cursor])
